...
public class NotificacionController {
    ...
    public ResponseEntity<?> listar(Authentication auth) {
        ...
        List<Map<String, Object>> items = notificacionService.listarUsuario(authUserId);
    ...
    public ResponseEntity<?> listarRecientes(
        ...
        Long authUserId = (Long) auth.getPrincipal();
        List<Map<String, Object>> items = notificacionService.listarUsuarioRecientes(authUserId, limit);
        return ResponseEntity.ok(items);
    }
    ...
    public ResponseEntity<?> contarNoLeidas(Authentication auth) {
        ...
        Long authUserId = (Long) auth.getPrincipal();
        long count = notificacionService.contarNoLeidas(authUserId);
    ...
    public ResponseEntity<?> marcarTodasLeidas(Authentication auth) {
        ...
        Long authUserId = (Long) auth.getPrincipal();
        notificacionService.marcarTodasLeidas(authUserId);
        return ResponseEntity.ok(Map.of("ok", true));
    }
}
